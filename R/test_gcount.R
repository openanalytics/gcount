...
#' 
#' @description Runs a set of tests to check if \code{gcount} is working 
#' correctly.
#'
#' @details Runs a series of tests. These can be used to see features and 
#' examples of usage, too. Running \code{test_gcount} will tell you the full 
#' location of the test file(s) to open.
#' @param verbose If \code{TRUE} sets \code{gcount_verbose} to \code{TRUE} for 
...
#' gcount:::test_gcount()
#' }
test_gcount <- function(verbose=FALSE, pkg="pkg", silent=FALSE) {
    ...
    for (fn in file.path(d, 'tests.Rraw')) {    # not testthat
        cat("Running", fn, "\n")
        oldverbose = getOption("gcount_verbose")
        if (verbose) options(gcount_verbose=TRUE)
        ...
        options(gcount_verbose=oldverbose)
    }
    invisible(sum(sapply(envirs, `[[`, "nfail"))==0)
}

# Define test() and its globals here, for use in dev
# But primarily called by test_gcount() calling inst/tests/tests.Rraw
...
test <- function(num, x, y, error=NULL, warning=NULL, output=NULL) {
    ...
    #    gcount appropriate definition of "equals" different
    ...
    # to cater for both test_gcount() and stepping through tests in dev
